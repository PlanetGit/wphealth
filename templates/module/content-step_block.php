...
<style>
    ...
    .arrow_down_on_hover:before {
        ...
        <?php if ($drag_bg_color === "bg-htcTurquoise") : ?>border-top: 15px solid var(--htcTurquoise);
        <?php else : ?>border-top: 15px solid var(--htcPurple);
        <?php endif; ?>position: absolute;
        right: 60%;
        margin-right: -10px;
        bottom: -13px;
    }
...
<script>
    ...
    $('input[id="range"]').on('mouseover', function() {
        ...
        if ($(window).width() > 640) {
            $('#drag_tooltip').css('left', `${activePos.left - (30 + index * 3)}px`);
        }
